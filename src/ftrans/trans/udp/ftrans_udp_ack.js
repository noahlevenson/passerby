...
class Ftrans_udp_ack {
  ...
  static is_valid_ack(buf) {
    ...
    if (buf.length !== Ftrans_udp_ack.ACKED_OFF + 1) {
      return false;
    }
  ...
  static get_acked(buf) {
    ...
    for (let i = 0; i < Ftrans_udp_ack.get_nslices(buf); i += 1) {
      acked.push((buf[Ftrans_udp_ack.ACKED_OFF] >> i) & 0x01 === 0x01 ? true : false);
    }
  ...
  static set_acked(buf, arr) {
    for (let i = 0; i < arr.length; i += 1) {
      if (arr[i]) {
        buf[Ftrans_udp_ack.ACKED_OFF] |= (0x01 << i);
      }
    }
