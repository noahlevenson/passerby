...
class Fstun_attr {
	...
	static K_ATTR_TYPE_TABLE = new Map([
		[new Buffer.from([0x00, 0x00]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0000, new Buffer.from([0x00, 0x00]))],
		[new Buffer.from([0x00, 0x01]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.MAPPED_ADDRESS, new Buffer.from([0x00, 0x01]), this._enMappedAddr)],
		[new Buffer.from([0x00, 0x02]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0002, new Buffer.from([0x00, 0x02]))],
		[new Buffer.from([0x00, 0x03]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0003, new Buffer.from([0x00, 0x03]))],
		[new Buffer.from([0x00, 0x04]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0004, new Buffer.from([0x00, 0x04]))],
		[new Buffer.from([0x00, 0x05]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0005, new Buffer.from([0x00, 0x05]))],
		[new Buffer.from([0x00, 0x06]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.USERNAME, new Buffer.from([0x00, 0x06]))],
		[new Buffer.from([0x00, 0x07]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_0007, new Buffer.from([0x00, 0x07]))],
		[new Buffer.from([0x00, 0x08]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.MESSAGE_INTEGRITY, new Buffer.from([0x00, 0x08]))],
		[new Buffer.from([0x00, 0x09]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.ERROR_CODE, new Buffer.from([0x00, 0x09]), this._enErrorCode)],
		[new Buffer.from([0x00, 0x0A]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.UNKNOWN_ATTRIBUTES, new Buffer.from([0x00, 0x0A]), this._enUnknownAttr)],
		[new Buffer.from([0x00, 0x0B]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.RESERVED_000B, new Buffer.from([0x00, 0x0B]))],
		[new Buffer.from([0x00, 0x14]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.REALM, new Buffer.from([0x00, 0x14]))],
		[new Buffer.from([0x00, 0x15]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.NONCE, new Buffer.from([0x00, 0x15]))],
		[new Buffer.from([0x00, 0x20]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.XOR_MAPPED_ADDRESS, new Buffer.from([0x00, 0x20]), this._enMappedAddr)],
		[new Buffer.from([0x80, 0x22]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.SOFTWARE, new Buffer.from([0x80, 0x22]), this._enSoftware)],
		[new Buffer.from([0x80, 0x23]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.ALTERNATE_SERVER, new Buffer.from([0x80, 0x23]))],
		[new Buffer.from([0x80, 0x28]).toString("hex"), new Fstun_type_data(this.K_ATTR_TYPE.FINGERPRINT, new Buffer.from([0x80, 0x28]))]
	]);
	...
	static K_ADDR_FAMILY_TABLE = new Map([
		[new Buffer.from([0x01]).toString("hex"), new Fstun_type_data(this.K_ADDR_FAMILY.IPv4, new Buffer.from([0x01]))],
		[new Buffer.from([0x02]).toString("hex"), new Fstun_type_data(this.K_ADDR_FAMILY.IPv6, new Buffer.from([0x02]))]
	]);
	...
	static _enMappedAddr(famType, addrStr, portInt, xor = false, id = Buffer.alloc(12)) {
		...
		const port = Futil._int2Buf16(portInt);
		let addr;


		if (famType === Fstun_attr.K_ADDR_FAMILY.IPv4) {
			addr = Futil._ipv4Str2Buf32(addrStr);
		} else if (famType === Fstun_attr.K_ADDR_FAMILY.IPv6) {
			addr = Futil._ipv6Str2Buf128(addrStr);
		}
	...
	static _decMappedAddr(buf, id, xor = false) {
		...
		if (famType.type === Fstun_attr.K_ADDR_FAMILY.IPv4) {
			decoded_addr = Futil._buf32_2_ipv4Str(addr);
		} else if (famType.type === Fstun_attr.K_ADDR_FAMILY.IPv6) {
			decoded_addr = Futil._buf128_2_ipv6Str(addr);
		}
