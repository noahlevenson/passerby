...
class Fstun_hdr {
	...
	static K_MSG_TYPE_TABLE = new Map([
		[new Buffer.from([0x00, 0x01]).toString("hex"), new Fstun_type_data(this.K_MSG_TYPE.BINDING_REQUEST, new Buffer.from([0x00, 0x01]))],
		[new Buffer.from([0x00, 0x11]).toString("hex"), new Fstun_type_data(this.K_MSG_TYPE.BINDING_INDICATION, new Buffer.from([0x00, 0x11]))],
		[new Buffer.from([0x01, 0x01]).toString("hex"), new Fstun_type_data(this.K_MSG_TYPE.BINDING_SUCCESS_RESPONSE, new Buffer.from([0x01, 0x01]))],
		[new Buffer.from([0x01, 0x11]).toString("hex"), new Fstun_type_data(this.K_MSG_TYPE.BINDING_ERROR_RESPONSE, new Buffer.from([0x01, 0x11]))]
	]);
	...
	static _isValidMsb(buf) {
		...
		if (Futil._getBit(buf, 0, 6) !== 0 || Futil._getBit(buf, 0, 7) !== 0) {
			return false;
		}
	...
	static _isValidMagic(magic) {
		return Futil._compareBuf(magic, this.K_MAGIC);
	}
