...
class Fapp {
	...
	static BOOTSTRAP_NODES = [
		new Ftrans_rinfo({
			...
			port: 27500,
			pubkey: Buffer.from('', "hex")
		})
	];
	...
	async start({addr = null, port = null} = {}) {
		...
		if (addr !== null && port !== null) {
			...
			for (let i = 0; i < Fapp.BOOTSTRAP_NODES.length && addr_port === null; i += 1) {
				addr_port = await fapp_stun_service._binding_req(Fapp.BOOTSTRAP_NODES[i].address, Fapp.BOOTSTRAP_NODES[i].port, Fapp.BOOTSTRAP_NODES[i].pubkey);
			}
		}
		...
		if (this.keepalive) {
			this.keepalive_interval_handle = setInterval(async () => {
				...
				for (let i = 0; i < Fapp.BOOTSTRAP_NODES.length && res === null; i += 1) {
					res = await fapp_stun_service._binding_req(Fapp.BOOTSTRAP_NODES[i].address, Fapp.BOOTSTRAP_NODES[i].port, Fapp.BOOTSTRAP_NODES[i].pubkey);
				}
