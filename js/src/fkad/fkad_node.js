/** 
* FKAD_NODE
* An FKAD_NODE is the nucleus of FKAD
* It implements the Kademlia protocol and wires together an FKAD_ENG
* with an FKAD_NET to provide a message engine and network I/O 
*
*
*/ 
...
class Fkad_node {
	...
	// net must be an Fkad_net net module, eng must be an Fkad_eng eng module
	...
	// we need to straighten out the design of this: how and why does Fkad_eng_alpha initiate the process
		...
		if (node_info !== null) {
			...
			this.network_data.entries().forEach((pair) => {
				...
				if (cnodes.includes(inbound_node_info) && 
						(Fkad_node._get_distance(this.node_id, key).less_equal(Fkad_node._get_distance(cnodes[0].node_id, key) || 
							cnodes[0] === inbound_node_info))) {
					
					Flog.log(`[FKAD] Replicating ${key.toString()} to new node ${inbound_node_info.node_id.toString()}`);
			...
			if (bucket === our_bucket) {
				...
				// to them... right?  Is it mathematically possible for that to fappen?
			}
		}
	}
	...
	async _node_lookup(key, rpc = this._req_find_node) {
		...
		function _by_distance_and_lex(node, oldnode) {
			if (Fkad_node._get_distance(key, node.get_data().node_id).less(Fkad_node._get_distance(key, oldnode.get_data().node_id))) {
				return -1;
			} else if (Fkad_node._get_distance(key, node.get_data().node_id).greater(Fkad_node._get_distance(key, oldnode.get_data().node_id))) {
				return 1;
			}
		...
		function _by_strict_equality(k, node) {
			...
			if (Fkad_node._get_distance(key, k.node_id).less(Fkad_node._get_distance(key, node_info.node_id))) {
				return -1;
			} else if (Fkad_node._get_distance(key, k.node_id).greater(Fkad_node._get_distance(key, node_info.node_id))) {
				return 1;
			}
		...
		this._new_get_nodes_closest_to(key, Fkad_node.ALPHA).forEach((node_info) => {
			inactive.bst_insert(new Fbintree_node({data: node_info}), _by_distance_and_lex.bind(this));
		});
	...
	_res_find_node(req) {
		const nodes = this._new_get_nodes_closest_to(req.data.payload[0], Fkad_node.K_SIZE);
	...
	_res_find_value(req) {
		...
		if (ds_rec) {
			...
			payload = this._new_get_nodes_closest_to(req.data.payload[0], Fkad_node.K_SIZE);
	...
	_new_get_nodes_closest_to(key, max = Fkad_node.K_SIZE) {
		...
		all_nodes.sort((a, b) => {
			return Fkad_node._get_distance(key, a.node_id).greater(Fkad_node._get_distance(key, b.node_id)) ? 1 : -1;
		});
	...
	_init_intervals() {
		...
		Flog.log(`[FKAD] K-bucket refresh interval: ${(Fkad_node.T_KBUCKET_REFRESH / 60 / 60 / 1000).toFixed(1)} hours`);
		...
		Flog.log(`[FKAD] Data republish interval: ${(Fkad_node.T_REPUBLISH / 60 / 60 / 1000).toFixed(1)} hours`);
		...
		if (this.replicate_interval_handle === null) {
			this.replicate_interval_handle = setInterval(() => {
				this.network_data.entries().forEach((pair) => {
					...
					if (t1 > (pair[1].get_created() + Fkad_node.T_REPLICATE) && t1 < (pair[1].get_created() + pair[1].get_ttl())) {
		...
		Flog.log(`[FKAD] Replication interval: ${(Fkad_node.T_REPLICATE / 60 / 60 / 1000).toFixed(1)} hours`);
	}
	...
	async bootstrap({addr = null, port = null, pubkey = null, node_id = null} = {}) {
		...
		Flog.log(`[FKAD] Joining network as ${this.node_id.toString()} via bootstrap node ${addr}:${port}...`);
		...
		Flog.log(`[FKAD] Success: node ${this.node_id.toString()} is online! (At least ${this._new_get_nodes_closest_to(this.node_id).length} peers found)`);
	...
	async put(key, val, rp = false) {
		...
		// Flog.log(`[FKAD] PUT key ${key.toString()} (${successful} / ${resolutions.length} OK)`)
