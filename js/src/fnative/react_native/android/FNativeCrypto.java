...
public class FNativeCrypto extends ReactContextBaseJavaModule {
	...
	// TODO: there's known issues with Java and PBES2 encryption - we're using unsafe PBES1 here because that's what works
	...
	public void signRSA(String data, String privkey, Promise promise)
		...
		InvalidKeyException, InvalidKeySpecException, IOException, NoSuchPaddingException {
			...
			try {
				...
				final byte[] privkey_bytes = FNativeCrypto.hex_str_to_bytes(privkey);
				PrivateKey key = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(privkey_bytes));
	...
	public void verifyRSA(String data, String pubkey, String sig, Promise promise) 
		...
		SignatureException {
			...
			try {
				...
				final PublicKey key = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(pubkey_bytes));
	...
	public void privateDecryptRSA(String data, String privkey, Promise promise) {
		try {
			final byte[] privkey_bytes = FNativeCrypto.hex_str_to_bytes(privkey);
			final PrivateKey key = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(privkey_bytes));
